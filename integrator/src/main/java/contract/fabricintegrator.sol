...
contract FabricIntegrator is owned, TokenERC20 {
    ...
    // Fabric balance
    mapping (address => uint256) public fabricBalanceOf;
    ...
    event FabricTransfer (address owner, uint256 value);

    /* This generates a public event on the blockchain that will notify clients */
    event FrozenFunds(address target, bool frozen);

    /* Initializes contract with initial supply tokens to the creator of the contract */
    function FabricIntegrator(
    ...
    /// we send tokens to Fabric
    function fabricTransfer(address _from, uint _value) public {
        require (_from != 0x0);                              // Prevent transfer to 0x0 address. Use burn() instead
        require (balanceOf[_from] >= _value);               // Check if the sender has enough
        require(!frozenAccount[_from]);                     // Check if sender is frozen
        balanceOf[_from] -= _value;                         // Subtract from the sender
        fabricBalanceOf[_from] += _value;                   // add to Fabric Balance

        FabricTransfer(_from, _value);                       // trigger event
